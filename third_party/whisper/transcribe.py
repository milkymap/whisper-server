    ...
    **decode_options,
    ...
    if decode_options.get("language", None) is None:
        if not model.is_multilingual:
            ...
            if verbose:
                logger.debug("Detecting language using up to the first 30 seconds. Use `--language` to specify the language")
            segment = pad_or_trim(mel, N_FRAMES).to(model.device).to(dtype)
            _, probs = model.detect_language(segment)
            decode_options["language"] = max(probs, key=probs.get)
            if verbose is not None:
                logger.debug(f"Detected language: {LANGUAGES[decode_options['language']].title()}")
    ...
    def add_segment(
        *, start: float, end: float, text_tokens: torch.Tensor, result: DecodingResult
        ...
        if verbose:
            message = f"[{format_timestamp(start)} --> {format_timestamp(end)}] {text}"
